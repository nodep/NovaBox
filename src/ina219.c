...
void ina_init(const bool max_bus_16v, const float maxCurrent)
{
	...
	// set the bus voltage range to 16V or 32V,
	// PGA/8, range 320mV
	// averaging to highest value (128 samples for bus and shunt voltage, full conversion time == 128ms)
...
{
	const uint16_t busVoltageRegister = ina_read(REG_BUS_VOLTAGE);
	const bool conversionReady = (busVoltageRegister & 2) != 0;
	
	if (conversionReady)
	{
		const uint16_t shuntVoltageRegister = ina_read(REG_SHUNT_VOLTAGE);
		const uint16_t currentRegister 		= ina_read(REG_CURRENT);
		const uint16_t powerRegister 		= ina_read(REG_POWER);
